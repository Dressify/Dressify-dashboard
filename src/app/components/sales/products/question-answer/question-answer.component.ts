...
export class QuestionAnswerComponent implements OnInit {
  ...
  constructor(public config: NgbRatingConfig, private sales:SalesService, private route:ActivatedRoute, private fb:FormBuilder, private router:Router) {
    config.max = 5;
  ...
  getQuestion(){
    ...
    if(id){
      const params = new HttpParams()
          .set('questionId', id)
      this.sales.getQuestionById(params).subscribe(data =>{
        this.question = data
  ...
  onSubmit(){
    ...
    if(this.answerQuestionForm.valid){
      ...
      this.sales.answerQuestion(this.answerQuestionForm.value).subscribe(data => {
        console.log(data)
        this.getQuestion()
      }, error => {
        console.log(error)
